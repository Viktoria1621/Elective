...
public class CourseRunUserInfoServiceImpl implements CourseRunUserInfoService {
    ...
    @Override
    public CourseRunUserInfo findCourseInfoByUser(int userID, int courseRunID) throws ServiceException {
        DAOFactory factory = DAOFactory.getInstance();
        CourseRunUserInfoDAO courseRunUserInfoDAO = factory.getCourseRunUserInfoDAO();
        CourseRunUserInfo courseRunUserInfo;

        try {
            courseRunUserInfo = courseRunUserInfoDAO.findCourseInfoByUser(userID, courseRunID);
        } catch (DAOException e) {
            throw new ServiceException("Course info isn't found.", e);
        }
        return courseRunUserInfo;
    ...
    @Override
    public List<CourseRunUserInfo> selectCourseRunInfo(int id) throws ServiceException {
        DAOFactory factory = DAOFactory.getInstance();
        CourseRunUserInfoDAO courseRunUserInfoDAO = factory.getCourseRunUserInfoDAO();
        List<CourseRunUserInfo> courseRunUserInfo;

        try {
            courseRunUserInfo = courseRunUserInfoDAO.selectCourseRunInfo(id);
        } catch (DAOException e) {
            throw new ServiceException("User course run info isn't found.", e);
        }
        return courseRunUserInfo;
    }
