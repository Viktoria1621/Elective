...
public class CourseRunDAOImpl implements CourseRunDAO {
    ...
    private static final String FIND_COURSE_RUN_BY_COURSE_REQUEST = "SELECT courseRun_id FROM courserun WHERE courses_id = ?";
    private static final String FIND_COURSE_RUN_COURSE_REQUEST = "SELECT courses_id FROM courserun WHERE courseRun_id = ?";
    private static final String FIND_COURSE_RUN_ID_BY_COURSE_ID_REQUEST = "SELECT courses_id FROM courserun WHERE courseRun_id = ?";
    ...
    @Override
    public List<Integer> findCourseRunIDByCourse(List<Integer> id) throws DAOException {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs;

        List<Integer> courseRunID = new ArrayList<>();

        try {
            con = connectionPool.takeConnection();
            ps = con.prepareStatement(FIND_COURSE_RUN_ID_BY_COURSE_ID_REQUEST);

            for (Integer c : id) {
                ps.setInt(1, c);
                rs = ps.executeQuery();
                while (rs.next()) {
                    courseRunID.add(rs.getInt("courseRun_id"));
                }
            }

        } catch (SQLException e) {
            throw new DAOException("Course run id isn't found.", e);
        }
    ...
    @Override
    public List<Integer> showCourseRunsByCourse(int id) throws DAOException {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs;
        ...
        try {
            con = connectionPool.takeConnection();
            ps = con.prepareStatement(FIND_COURSE_RUN_BY_COURSE_REQUEST);
